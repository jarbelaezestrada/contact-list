...
function actualizarContacto(id, nuevosDatos) {
    ...
    const indice = listContact.findIndex(contacto => contacto.id === id);
    if (indice !== -1) {
        //Actualizacion de datos de contacto
        listContact[indice] = {...listContact [indice], ...nuevosDatos}; 
        console.log("Contacto con id ${id} actualizado.");   
    }else {
        console.log("No se encontro contacto con id");
    }
}

actualizarContacto(1013, {name: "Belen", telefono: 573203875654});
imprimir();
